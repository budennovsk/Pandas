...
def PCA(num_comp):
    ...
    pca = UMAP(n_components=num_comp,random_state=42)
...
def new_cat_model(res_PCA, pca,head_df,limit, limit_min):
    train = res_PCA.sample(frac=0.8).copy()
...
def test_all(model, data, pca, head_df, limit, limit_min):
    ...
    min_v = limit_min.tolist()[0]
    max_v = limit.tolist()[0]
    ...
    print("РЕЗУЛЬТАТ")
    print(pd.DataFrame(pca.inverse_transform(result.x.reshape(1, -1))))
